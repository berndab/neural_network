...
Philanthropic organizations apply for funding and receive monetary grants to create and manage charitable projects that fulfill necessary community needs. These projects range from land preservation to healthcare initiatives . The type of organizations that apply for these grants range from trusts to cooperatives. Organizations that give these grants must evaluate the history of the organization that is applying for these grant, the types of project that this organization has manages, and the outcomes of these projects in order to determine if the organization meets their criteria to receive a grant. The goal of this project is to create a neural network based, machine learning model to assist in this evaluation process and make it more efficient and accurate.


This neural network machine learning model will be trained and tested on a data set that contains information about past charitable projects that were funded by grants. This data set contains fields that categorize the type of organization that managed a specific project, the type of project managed, the dollar value of the grant issued for the project, and the outcome of the project. When the model is used in the grant evaluation process, the grant application will have the values for these features which will be inputted into the model to produce a outcome prediction for the project proposed in the application.


## Model Development Modular Design Approach

A modular design approach was used to segregate the code that is required to execute each step in the model development processes. The model development process contains process steps that only need to be run once during model development. If the code for all process steps is contained in one Jupyter Notebook then all the process steps must be run each time a model training and testing cycle is run, even the process steps that only need to be run once, like the feature analysis step and the feature elimination step. If the programing code these run one time only process steps are separated into their own standalone notebook modules, then only the training and testing notebook module needs to be executed to run a modle training and testing cycle. In addition, each model training and testing cycle will run faster as a standalone notebook model because it will not contain preprocessing code that only need to be executed once. Faster execution of model training and testing cycles allow for more cycles can be run during the model training and testing phase using various model configurations to find the model configuration that has the highest accuracy.

Furthermore, by creating  a standalone module for the for static features encoding process step addition static feature encoding modules can be develop that use different feature encoding algorithms ,such as LabelEncoder, to determine if these other encoding algorithms create a more accurate model. It was not possible to create additional encoding modules using different encoding algorithms within the time frame allotted to create this model. Finally, with the training and testing code in a stand alone notebook module the model the python code in the notebook module can be exported to a Python file that can be modified to enable automated testing of the model using different configuration variable values to determine the set of values that produce the highest model accuracy.  
